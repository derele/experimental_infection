...
good.contigs <- contig.df[contig.df$Ac, "contig"]
MN.contigs <- contig.df[contig.df$AcMN, "contig"]

counts.frame <- counts.frame[ rownames(counts.frame)%in%good.contigs, ]

counts.frame.MN <- counts.frame[ rownames(counts.frame)%in%MN.contigs, ]

co.f <- counts.frame[rowSums(counts.frame)>32, ]

co.f.MN <- co.f[rownames(co.f)%in%MN.contigs,]

mapped.good <- colSums(counts.frame.MN)

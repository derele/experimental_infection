...
rep.conds[rep.conds!=c("first", "second")] <- "third"
...
sig.rep1 <- res.rep1[res.rep1$padj < .1 & !is.na(res.rep1$padj),"id"]
sig.rep2 <- res.rep2[res.rep2$padj < .1 & !is.na(res.rep2$padj),"id"]
sig.rep3 <- res.rep3[res.rep3$padj < .1 & !is.na(res.rep3$padj),"id"]
### ohh damn this is a long list of bullshit 
shit <- length(unique(c(sig.rep1, sig.rep2, sig.rep3)))
@ 

...
## rep.design <- model.matrix(~sex.conds*eel.conds*rep.conds)
## rep.ed <- DGEList(co.f, lib.size=colSums(co.f))
## rep.ed <- calcNormFactors(ed)
...
## glm.rep <- glmLRT(rep.ed, rep.glmfit, grep("rep", colnames(rep.glmfit$design)))
...
## rep.design <- model.matrix(~sex.conds*eel.conds*rep.conds)
## rep.ed.MN <- DGEList(co.f, lib.size=colSums(co.f))
## rep.ed.MN <- calcNormFactors(ed.MN)
## rep.ed.MN <- estimateGLMCommonDisp(rep.ed.MN, design=rep.design)
## rep.ed.MN <- estimateGLMTrendedDisp(rep.ed.MN, design=rep.design)
## rep.ed.MN <- estimateGLMTagwiseDisp(rep.ed.MN, design=rep.design)
## rep.glmfit.MN <- glmfit(rep.ed.MN, rep.design, dispersion=rep.ed.MN$tagwise.dispersion)

## glm.rep <- glmLRT(rep.ed.MN, rep.glmfit.MN, grep("rep", colnames(rep.glmfit.MN$design)))
## topTags.rep <- topTags(glm.rep, n=40000) ## set as high as the length 
## topTags.rep <- topTags.rep[topTags.rep$table$adj.P.Val<0.05] 
...
pop.to.plot.male <- vsd[all.methods.pop.contigs, ]
